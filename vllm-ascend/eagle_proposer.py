...
class EagleProposer(VllmEagleProposer):
    ...
    def __init__(self,
        ...
        # ------------------------------------------------------------------
        # Tree-attention fields for EAGLE tree-structured speculation.
        # Activated when speculative_token_tree defines a branching tree.
        # ------------------------------------------------------------------
        self._use_tree_attention = False
        self.tree_attn_metadata_builder = None
        self._tree_common_attn_metadata = None  # set in _propose() for tree mode

        spec_token_tree = getattr(self.speculative_config, "speculative_token_tree", None)
        if spec_token_tree is not None:
            _tree_choices: list[tuple[int, ...]] = ast.literal_eval(spec_token_tree)
            # A non-trivial tree has branching (>1 child) or multiple levels.
            if len(_tree_choices) > 1:
                self._use_tree_attention = True
                # Tree mode is incompatible with ACL graph capture due to
                # variable token counts per tree level.
                self.use_cuda_graph = False

                # Per-level properties (same as GPU vllm EagleProposer).
                _tree_depth = len(_tree_choices[-1])
                _num_per_level = [0] * _tree_depth
                for _node in _tree_choices:
                    _num_per_level[len(_node) - 1] += 1
                self.cu_drafts_per_level = [_num_per_level[0]]
                self.child_drafts_per_level = [_num_per_level[0]]
                for _lv in range(1, _tree_depth):
                    self.cu_drafts_per_level.append(
                        self.cu_drafts_per_level[-1] + _num_per_level[_lv])
                    self.child_drafts_per_level.append(
                        _num_per_level[_lv] // _num_per_level[_lv - 1])
                _max_bs = vllm_config.scheduler_config.max_num_seqs
                # Position offsets for each tree node relative to root.
                self.tree_draft_pos_offsets = torch.arange(
                    1, len(_tree_choices) + 1, device=device, dtype=torch.int32
                ).repeat(_max_bs, 1)
    ...
    def _get_attention_metadata_builder(self):
        """Return the attention metadata builder for draft model layers.

        When tree attention is configured, return an
        AscendTreeAttentionMetadataBuilder that builds tree-aware metadata.
        Otherwise, fall back to the base-class behaviour (runner's builder).
        """
        if self._use_tree_attention:
            if self.tree_attn_metadata_builder is None:
                from vllm_ascend.attention.tree_attn_v1 import (  # noqa: PLC0415
                    AscendTreeAttentionMetadataBuilder,
                )
                # Retrieve kv_cache_spec / layer_names from the runner's builder.
                base_builder = None
                ...
                assert base_builder is not None, (
                    "Failed to find base attention metadata builder for "
                    "AscendTreeAttentionMetadataBuilder construction."
                )
                self.tree_attn_metadata_builder = AscendTreeAttentionMetadataBuilder(
                    kv_cache_spec=base_builder.kv_cache_spec,
                    layer_names=base_builder.layer_names,
                    vllm_config=self.vllm_config,
                    device=self.device,
                )
            return self.tree_attn_metadata_builder
        return super()._get_attention_metadata_builder()
    ...
    def _propose(
        ...
        # For tree-attention mode: store common metadata so that
        # _run_merged_draft can pass it to propose_tree().
        if self._use_tree_attention:
            self._tree_common_attn_metadata = copy.copy(common_attn_metadata)
        else:
            # Copy the old attn_metadata and update for each linear draft step.
    ...
    def _run_merged_draft(self,
        ...
        if self.uses_mrope:
            ...
            positions = target_positions[last_token_indices]
        hidden_states_root = hidden_states[last_token_indices]

        # ------------------------------------------------------------------
        # Tree-attention path: multi-path EAGLE tree proposal.
        # ------------------------------------------------------------------
        if self._use_tree_attention and self._tree_common_attn_metadata is not None:
            draft_token_ids_list = self.propose_tree(
                batch_size=batch_size,
                logits=logits,
                positions=positions,
                hidden_states=hidden_states_root,
                common_attn_metadata=self._tree_common_attn_metadata,
            )
            # [batch_size, total_tree_nodes]
    ...
    def propose_tree(
        ...
        """
        Generate tree-structured draft tokens for EAGLE speculative decoding.

        This implements the multi-level tree sampling where, at each depth,
        all candidate tokens across requests are processed in a single forward
        pass of the draft model with tree-aware attention.

        Args:
            batch_size:           Number of active requests.
            logits:               [batch_size, vocab_size] from depth-0 pass.
            positions:            [batch_size] root token positions.
            hidden_states:        [batch_size, hidden_size] from depth-0 pass.
            common_attn_metadata: Snapshot of attention metadata at depth-0.

        Returns:
            List of [batch_size, branching_factor] tensors, one per tree depth.
            Caller concatenates them to get [batch_size, total_tree_nodes].
        """
        from vllm_ascend.attention.tree_attn_v1 import (  # noqa: PLC0415
            AscendTreeAttentionMetadataBuilder,
        )

        tree_builder = self._get_attention_metadata_builder()
        assert isinstance(tree_builder, AscendTreeAttentionMetadataBuilder)

        total_num_drafts = self.cu_drafts_per_level[0]
        level_num_drafts = total_num_drafts
        num_children = self.child_drafts_per_level[0]

        # Sample draft tokens for tree root children (depth-0 output).
        if num_children == 1:
        ...
        # draft_hidden_states: [batch_size, num_at_this_level, hidden_size]
        draft_hidden_states = hidden_states.view(batch_size, 1, -1)

        # Accumulators for the current tree prefix inputs.
        tree_input_ids = torch.empty(
            0, device=self.input_ids.device, dtype=self.input_ids.dtype)
        tree_positions = torch.empty(
            0, device=self.device, dtype=positions.dtype)
        tree_hidden_states = torch.empty(
            0, device=self.device, dtype=self.dtype)

        # Precompute all draft token position offsets for the whole tree:
        # tree_draft_pos_offsets[b, i] = offset of the i-th tree node.
        ...
        block_size = tree_builder.kv_cache_spec.block_size

        for level in range(tree_depth - 1):
            # Positions for queries at this level (broadcast over level nodes).
            draft_positions = (positions + (level + 1)).view(batch_size, -1)
            exceeds_max = (
                (positions + total_num_drafts) >= self.max_model_len
            )
            draft_positions = torch.where(
                exceeds_max, torch.zeros_like(draft_positions), draft_positions
            )
            ...
            # Accumulate inputs for all nodes up to and including this level.
            ...
            # Build tree-attention metadata for this level.
            query_len = total_num_drafts
            # Clone seq_lens to avoid in-place modification of the root snapshot.
            new_seq_lens = common_attn_metadata.seq_lens[:batch_size].clone() \
                + level_num_drafts
            new_seq_lens_cpu = (
                common_attn_metadata.seq_lens_cpu[:batch_size].clone()
                + level_num_drafts
            )

            tree_common = copy.copy(common_attn_metadata)
            tree_common.num_reqs = batch_size
            tree_common.seq_lens = new_seq_lens
            tree_common.seq_lens_cpu = new_seq_lens_cpu
            tree_common.num_actual_tokens = batch_size * query_len
            tree_common.num_input_tokens = batch_size * query_len
            tree_common.max_query_len = query_len
            tree_common.query_start_loc = (
                query_len * self.arange[: batch_size + 1]
            )
            tree_common.query_start_loc_cpu = torch.arange(
                batch_size + 1, dtype=torch.int32
            ) * query_len
            tree_common.actual_seq_lengths_q = (
                tree_common.query_start_loc_cpu[1:].tolist()
            )
            tree_common.attn_state = AscendAttentionState.ChunkedPrefill

            attn_metadata = tree_builder.build_for_drafting(
                tree_common, draft_index=level + 1
            )

            # Clamp seq_lens for requests that exceed max_model_len.
            attn_metadata.seq_lens.masked_fill_(exceeds_max, 1)
            attn_metadata.seq_lens_list = attn_metadata.seq_lens[:batch_size].tolist()

            # Compute slot mapping for tree nodes at this level.
            # flattened_draft_positions[:, level : level + query_len] gives
            # the absolute positions of the tree nodes being written to cache.
            query_positions = flattened_draft_positions[:, level: level + query_len]
            block_numbers = query_positions // block_size
            block_ids = attn_metadata.block_tables[:batch_size].gather(
                dim=1, index=block_numbers
            )
            slot_mapping = block_ids * block_size + query_positions % block_size
            slot_mapping[exceeds_max] = PADDING_SLOT_ID
            attn_metadata.slot_mapping = slot_mapping.view(-1).to(torch.int32)

            # Copy inputs into persistent buffers.
            ...
            # Run draft model for this tree level.
            with set_ascend_forward_context(
                per_layer_meta,
                self.vllm_config,
                num_tokens=num_tokens,
            ...
            if self.method == "mtp":
                last_hs = ret_hidden
                hidden_states_out = last_hs
            else:
                last_hs, hidden_states_out = ret_hidden

            # Extract hidden states for the NEW nodes added at this level.
            draft_hidden_states = hidden_states_out[:num_tokens].view(
                batch_size, query_len, -1
            )[:, -level_num_drafts:]
            draft_last_hs = last_hs[:num_tokens].view(
                batch_size, query_len, -1
            )[:, -level_num_drafts:]

            # Compute logits and sample next-level draft tokens.
            logits = self.model.compute_logits(
                draft_last_hs.reshape(batch_size * level_num_drafts, -1)
            )
            ...
            level_num_drafts = (
                self.cu_drafts_per_level[level + 1] - total_num_drafts
            )
