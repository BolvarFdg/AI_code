...
class AscendAttentionBackendImpl(AttentionImpl):
    ...
    def forward_impl(
        ...
        # Tree-attention path for EAGLE speculative decoding draft model.
        # Import lazily to avoid a circular import at module load time.
        from vllm_ascend.attention.tree_attn_v1 import (  # noqa: PLC0415
            AscendTreeAttentionMetadata,
            forward_tree_decode_attention,
        )
        if (
            isinstance(attn_metadata, AscendTreeAttentionMetadata)
            and attn_metadata.tree_attn_bias is not None
            and attn_metadata.tree_len > 1
            and self.key_cache is not None
        ):
            return forward_tree_decode_attention(
                query=query,
                key_cache=self.key_cache,
                value_cache=self.value_cache,
                ...
                tree_len=attn_metadata.tree_len,
                num_heads=self.num_heads,
                num_kv_heads=self.num_kv_heads,
                scale=self.scale,
                output=output,
